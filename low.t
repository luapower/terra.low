...
--capture ffi cdefs generated by terra ---------------------------------------

local capture
local function start_capture_cdef()
	capture = {}
end
local function end_capture_cdef()
	local s = table.concat(capture, '\n')
	capture = false
	return s
end
...
ffi.cdef = function(s)
	if capture then
		if s:find'^struct ' then
			s = s:gsub('{ ', '{\n\t'):gsub('; ([^}])', ';\n\t%1'):gsub('; }', ';\n}')
		end
		table.insert(capture, s)
	end
	return oldcdef(s)
end
start_capture_cdef()
...
low.start_capture_cdef = start_capture_cdef
low.end_capture_cdef = end_capture_cdef
...
function low.addmethods(T, addmethods_func)
	...
	--in case meta-code tries to look up a method in arr.methods ...
...
function low.function_cdef(f, type_name, method_name)
	local t = f:gettype()
	local pa = t.parameters:map(getcstring)
	pa = concat(pa, ', ')
	pa = t.isvararg and pa..',...' or pa
...
function low.methods_cdef(T, saveobj_table)
	...
	for name, m in sortedpairs(T.methods, cmp) do
		if not name:starts'_' then --skip private methods
			...
			saveobj_table[cname] = m
		end
	end
	add(t, ']]')
	add(t, 'ffi.metatype(\''..T.name..'\', {__index = {')
	add(t, concat(kt)..'}})')
	return concat(t, '\n')
end
...
return C
	...
	terralib.saveobj(name..'.o', 'object', saveobj_table)

	local soext = {Windows = 'dll', OSX = 'dylib', Linux = 'so'}
	local soname = name..'.'..soext[ffi.os]
	local deps = conf.linkto and '-l'..concat(conf.linkto, ' -l') or ''

	os.execute('gcc '..name..'.o -shared -o '..soname..' -L. '..deps)
end
